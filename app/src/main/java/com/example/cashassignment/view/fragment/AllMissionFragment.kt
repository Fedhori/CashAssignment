...
class AllMissionFragment : Fragment() {
    ...
    private fun initViewPager(){
        ...
        for(category in TaskCategory.values()){
            homeViewModel.getTaskData(category).observe(viewLifecycleOwner, androidx.lifecycle.Observer { taskList ->
                missionFragmentViewPagerAdapter.submitList(category.ordinal, taskList as MutableList<TaskEntity>?)
            })
        }
